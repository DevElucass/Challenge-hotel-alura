package com.github.hotelalura.views;
...
import com.github.hotelalura.jdbc.Controller;
import com.github.hotelalura.jdbc.Guest;
import com.github.hotelalura.jdbc.Reserve;

@SuppressWarnings("serial")
public class MenuSearch extends JFrame {
	private JPanel contentPanel;
	private String imgPath = "/com/github/hotelalura/images/";
	...
	public MenuSearch() {
		...
		tbGuestsModel = new DefaultTableModel(new String[] {
				"ID do h�spede", "Nome", "Sobrenome",
				"Data de Nascimento", "Pa�s", "Telefone", "ID da reserva"}, 0);
			...
			public void mouseClicked(MouseEvent e) {
				try {
					if(guests.size() == tbGuestsModel.getRowCount()) {
						if(JOptionPane.showConfirmDialog(contentPanel, "Deseja salvar as altera��es?") == 0) {
							for(int i = 0; i < guests.size(); i++) {
								guests.get(i).update(
										...
										tbGuestsModel.getValueAt(i, tbGuestsModel.findColumn("Pa�s")).toString(), 
					...
					JOptionPane.showMessageDialog(contentPanel, "N�o h� registros para editar");
				}
			}
			@Override
			public void mouseEntered(MouseEvent e) {
				...
				if(target != -1) {
					if(JOptionPane.showConfirmDialog(contentPanel, "Deseja deletar o registro?") == 0)
						try {
							...
							JOptionPane.showMessageDialog(contentPanel, "N�o foi poss�vel deletar o registro");
						}
				}
				else {
					JOptionPane.showMessageDialog(contentPanel, "N�o h� registro selecionado");
				}
			}
			@Override
			public void mouseEntered(MouseEvent e) {
